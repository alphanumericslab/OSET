% Event related potential extraction using nonstationary component analysis
%
...
% NSCA
erp_effectivelen = round(0.5*fs);
pulse_target = zeros(1, T);
pulse_target(target_indexes) = 1;
ppulse_target = filter(ones(erp_effectivelen, 1), 1, pulse_target);
ppulse_target(70000:71500) = 0; % Force remove the components corresponding to the huge peak    <<============
I_targets = find(ppulse_target);

pulse_nontarget = zeros(1, T);
pulse_nontarget(nontarget_indexes) = 1;
ppulse_nontarget = filter(ones(erp_effectivelen, 1), 1, pulse_nontarget);
ppulse_nontarget(70000:71500) = 0; % Force remove the components corresponding to the huge peak    <<============
I_nontargets = find(ppulse_nontarget);

figure
hold on
plot(pulse_target);
plot(ppulse_target, 'r');
grid
...
for i = 1:N
    ...
    %     errorbar(tt, X_targets_mn(i, :), sqrt(X_targets_vr(i, :))/2, 'k');
