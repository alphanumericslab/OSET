...
%//////////////////////////////////////////////////////////////////////////
% General parameters
len = 120; % length of ECG in seconds
fs  = 500;       % desired sampling rate
N   = round(fs*len);       % # of signal samples
hr  = 110;
...
% Twave parameters
twafx = 0.999;% 0.9; % factor by which to modify T-wave in x-dirn
twafy = 0.997;% 0.9; % factor by which to modify T-wave in y-dirn
twafz = 0.995;% 0.9; % factor by which to modify T-wave in z-dirn
...
% Dipole parameters
F = .9;                     % heart rate
% generate an RR interval time series
rr = rrprocess(ceil(hr*len/60),hr, 2, 5);

% and add a linear trend to RR intervals (in hr space, not rr space) +/- ramp/2 bpm
hrv = 60./rr;
ramp = 20;
tmp=[1:1:length(rr)];tmp=ramp*tmp/max(tmp);
tmp=tmp-mean(tmp); tmp=tmp';
hrv=hrv+tmp;
F = 60./hrv;
%plot(hrv);title('hrv')
%rrc=resample(hrv,N,length(hrv));
%F=60./rrc;
...
% 
% tetai(2).x  = [-1.09  -0.83   -0.19     -.07  0 .06        0.22    1.2 1.42 1.68 2.9];
% alphai(2).x = [0.03   .08    -0.13    .85 1.11 .75     0.06   0.1  0.17 0.39 .03];
% bi(2).x     = [0.0906    0.1057    0.0453    0.0378    0.0332    0.0302    0.0378    0.6040 0.3020  0.1812 .5];
% 
% tetai(2).y  = [-1.1  -0.9 -0.76       -0.11   -.01       0.065  0.8 1.58 2.9];
% alphai(2).y = [0.035 0.015 -0.019     0.32    .51     -0.32    0.04   0.08 .014];
% bi(2).y     = [0.07  .07  0.04        0.055    0.037    0.0604  0.450  0.3 .5];
% 
% tetai(2).z  = [-1.1  -0.93 -0.7      -.4     -0.15    .095    1.05 1.25 1.55 2.8];
% alphai(2).z = [-0.03 -0.14 -0.035    .045     -0.4    .46    -.12 -.2 -.35 -.035];
% bi(2).z     = [.03  .12  .04         .4    .045       .05    .8 .4 .2 .4];
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[DIP, teta] = DipoleGeneratorAbnormal3(N,fs,F,alphai,bi,tetai,teta0,STM,S0);
