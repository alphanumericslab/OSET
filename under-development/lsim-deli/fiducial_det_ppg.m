...
for f_hp= 0.5:0.1:1.5
    ...
    if sum(a(1:ind_15))/sum(a(1:ind_10))<0.25
...
for p = 1:length(ecg_rpeaks_index)-1

    if ecg_rpeaks_index(p)>1430450-250
        yu=0;
    end
    ...
    [TF,P] = islocalmax(dppg(temp_index),'MaxNumExtrema',1);
    TF = find(TF);
    if isempty(TF)
        continue;
    end
    ...
    if isempty(TFdn) || data(temp_index(TFdn))<thr_5perc
        ...
        if ~isempty(TFdw)
            ...
            [TFdn,Pdn] = islocalmin(data_detrended,'MaxNumExtrema',1);
            TFdn = find(TFdn);

            [TFdias,Pdn] = islocalmax(data_detrended,'MaxNumExtrema',1);
            TFdias = find(TFdias);
...
if flag_post_processing>0
    ...
    ind_c = 1:length(dur_qrs);
    dur_qrs(ind_c) = em_interval_calc([zeros(length(ind_c),1),dur_qrs(ind_c)'],0.20,0.15,180);
    ...
    positions.onset = sys_peak_index(:)' - dur_qrs;
    ...
    dur_qrs = round(dur_qrs-max(0,0.1*fs-md_temp));
    positions.u_peak = sys_peak_index(:)' - dur_qrs;
    ...
    dur_qrs = round(dur_qrs-max(0,0.1*fs-md_temp));
    positions.w_peak  = sys_peak_index(:)' + dur_qrs;
    ...
    dur_qrs = round(dur_qrs-max(0,0.1*fs-md_temp));
    positions.d_notch  = sys_peak_index(:)' + dur_qrs;
    ...
    dur_qrs = round(dur_qrs-max(0,0.1*fs-md_temp));
    positions.dias_peak  = sys_peak_index(:)' + dur_qrs;
