function [data_posterior_est, data_prior_est, n_var] = ecg_den_phase_domain_gp_full_cov(data, I_peaks, params)

if ~isfield(params, 'num_phase_bins') % number of phase bins
    params.num_phase_bins = ceil(max(diff(I_peaks))); % use max RR-interval by default
end
if mod(params.num_phase_bins, 2) == 0
    params.num_phase_bins = params.num_phase_bins + 1;
end

if ~isfield(params, 'nvar_est_method') % noise variance estimation method
    params.nvar_est_method = 'MIN'; % Min variance across average beat, by default
end
...
% data_padded = [zeros(num_ch, left_padding), data, zeros(num_ch, right_padding)];
for ch = 1 : num_ch
    x_stacked = event_stacker(data(ch, :), I_peaks, params.num_phase_bins);

    % x_stacked = zeros(length(II_peaks), params.num_phase_bins);
    beat_seg_borders = round((I_peaks(1:end-1) + I_peaks(2:end)) / 2);
    % while beat_seg_borders(1) > 1
    %     beat_seg_borders = cat(2, round(beat_seg_borders(1) - (I_peaks(1) + I_peaks(2)) / 2), beat_seg_borders);
    % end
    % 
    % while beat_seg_borders(end) < sig_len
    %     beat_seg_borders = cat(2, beat_seg_borders, round(beat_seg_borders(end) + (I_peaks(end) + I_peaks(end-1)) / 2));
    % end

    % potential_first_mid_point = beat_seg_borders(1) - (beat_seg_borders(2) - beat_seg_borders(1));
    % beat_seg_borders = cat(2, potential_first_mid_point, beat_seg_borders);
    %
    % potential_last_mid_point = beat_seg_borders(end) + (beat_seg_borders(end) - beat_seg_borders(end-1));
    % beat_seg_borders = cat(2, beat_seg_borders, potential_last_mid_point);
    %
    % Phi = zeros(beat_seg_borders(1)-1, params.num_phase_bins);
    Phi = [];
    phi_k = cell(1, num_peaks);
    for k = 1 : num_peaks
        if k == 1
            first_index = beat_seg_borders(1) - (I_peaks(2) - I_peaks(1));
        else
            first_index = beat_seg_borders(k-1);
        end
        if k < num_peaks
            last_index = beat_seg_borders(k) - 1;
        else
            last_index = beat_seg_borders(end) - 1 + (I_peaks(end) - I_peaks(end-1));
        end
        % beat = data(ch, first_index : last_index);
        theta_k = warping_transform_col([first_index, I_peaks(k), last_index] - first_index + 1, [1, ceil(params.num_phase_bins/2), params.num_phase_bins], params.interp_order);
        phi_k{k} = pinv(theta_k);
        if first_index < 1
            first_index
        end

        if last_index > sig_len
            last_index
        end
    ...
    % Phi = cat(1, Phi, zeros(size(data_padded, 2) - size(Phi, 1), params.num_phase_bins));
    ...
    Phi = Phi(left_padding + 1 : left_padding + sig_len, :);
    ...
    % [ECG_mean_0, ECG_std_0, meanphase_0, ECG_median_0, ECGSamplesPerBin_0] = avg_beat_calculator_phase_domain(data_padded, phase, params.num_phase_bins, 0); % mean ECG extraction
    ...
    switch params.nvar_est_method
        case 'GIVEN'
            params.nvar_factor = 1;
            noise_std_est = sqrt(params.nvar);
    ...
    switch METHOD
        ...
        case 'PHASE_DOMAIN_GP_IMPL'
            data_prior_est_padded = [];%zeros(beat_seg_borders(1)-1, 1);
            data_post_est_padded = [];%zeros(beat_seg_borders(1)-1, 1);
            ...
            data_prior_est_padded = cat(1, zeros(beat_seg_borders(1)-1, 1), data_prior_est_padded);
            data_post_est_padded = cat(1, zeros(beat_seg_borders(1)-1, 1), data_post_est_padded);

            sig_len
            left_padding
            size(data_prior_est_padded)
            data_prior_est_padded = data_prior_est_padded(left_padding + 1 : left_padding + sig_len)';
            data_post_est_padded = data_post_est_padded(left_padding + 1 : left_padding + sig_len)';
    end

    data_prior_est(ch, :) = data_prior_est_padded;%(left_padding + 1 : left_padding + sig_len)';
    data_posterior_est(ch, :) = data_post_est_padded;%(left_padding + 1 : left_padding + sig_len)';
    ...
    % plot((eye(sig_len) - A) * data_prior_est_padded, 'linewidth', 1); lgnd = cat(2, lgnd, '(I - A)*\mu');
...
if 1
    ...
    plot(nn_padded(beat_seg_borders), data_padded(ch, beat_seg_borders), 'gx', 'markersize', 18);
    grid
...
%     x_stacked = zeros(length(end_of_beat_indexes), params.num_phase_bins);
...
%         x_stacked(ll, :) = LinearWarp(x(beat_start : beat_end), params.num_phase_bins);%x(beat_start : beat_end) * (diag(max(1, sum(M(:, beat_start : beat_end), 2))) \ M(:, beat_start : beat_end))';
...
%     %
%     [~, ~, meanphase, ~, ECGSamplesPerBin] = MeanECGExtraction(x, phase, params.num_phase_bins, 1); % mean ECG extraction
%
...
%     % reconstruct the ECG
%     if 0 % under dev.
%         K_s_ph = K_x_ph - n_var * eye(params.num_phase_bins);
%         rho = K_s_ph / n_var;
%         sig_len = length(x);
...
%         term2 = T2' * rho;
%         term3 = T2 * T2';
%         term3_ = tril(triu(term3, -round(params.num_phase_bins/2)), round(params.num_phase_bins/2)); % removes the phase glithces at the corners of term3
...
%             plot(meanphase, sqrt(n_var)*ones(1, params.num_phase_bins));
%             grid
