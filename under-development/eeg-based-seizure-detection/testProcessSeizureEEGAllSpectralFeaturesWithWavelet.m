...
clear;
clc;
resultfname = 'testProcessSeizureEEGAllSpectralFeatures4.txt';
resultfname_wavelet = 'testProcessSeizureEEGSpectralWaveletFeatures4.txt';
...
for m = 1 : length(subject),
    ...
    for k = 1 : NumRecords,
        ...
        for i = 1 : length(feature)
            fprintf(fid, '\t%12.8f', feature(i));
        end
        for i = 1 : length(feature_cross)
            fprintf(fid, '\t%12.8f', feature_cross(i));
        end
        for i = 1 : length(feature_eigs)
            fprintf(fid, '\t%12.8f', feature_eigs(i));
        end
        fprintf(fid, '\n');
        fclose(fid);
        ...
        for i = 1 : length(waveletfeaturesCf)
            fprintf(fid_wavelet, '\t%12.8f', waveletfeaturesCf(i));
        end
        for i = 1 : length(waveletfeaturesCfcross)
            fprintf(fid_wavelet, '\t%12.8f', waveletfeaturesCfcross(i));
        end
        fprintf(fid_wavelet, '\n');
        fclose(fid_wavelet);
