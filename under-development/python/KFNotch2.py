...
def KFNotch2(x, f0, fs, Qbar=[], Wlen=[]):
    # input arguments
    if not Qbar:
        Qbar = 0.0001 * max([abs(i) for i in x])

    if not Wlen:
        Wlen = int(np.ceil(0.1 * fs))
    ...
    for k in range(Samples):
        ...
        K = np.divide(
            np.dot(Pminus, np.transpose([H])),
            (np.dot(np.dot(H, Pminus), np.transpose([H])) + R),
        )
        Pplus = np.dot(
            np.dot((np.eye(L) - np.dot(K, [H])), Pminus),
            np.transpose(np.eye(L) - K * H),
        ...
        Pminus = np.dot(np.dot(A, Pplus), np.transpose(A)) + np.dot(
            B * Q, np.transpose(B)
        )
    ...
    for k in reversed(range(Samples - 1)):
        S = np.dot(
            np.dot(Phat[:, :, k], np.transpose(A)), np.linalg.inv(Pbar[:, :, k + 1])
        )
        X[:, k] = Xhat[:, k] + np.dot(S, (X[:, k + 1] - Xbar[:, k + 1]))
        PSmoothed[:, :, k] = Phat[:, :, k] - np.dot(
            np.dot(S, (Pbar[:, :, k + 1] - PSmoothed[:, :, k + 1])), np.transpose(S)
        )
