...
for ttp = 1 : length(type),
    switch(type{ttp})
        case 'Normal'
            ...
            seg_len = 10;
            NormalizedCutOff = linspace(5.0/fs, 0.5, 100);
        case 'Arrhythmia'
            ...
            diff_h = [2 4 6];
            seg_len = 10;
            %             NormalizedCutOff = linspace(.001, .45, 250);
            NormalizedCutOff = linspace(5.0/fs, 0.5, 100);
        case 'PTB'
            pth = 'J:\ECGData\PTB';
            fs = 1000;
    ...
    fresults = ['AllDataSmootherResults' type{ttp} '05_ComparisonWithLPF.txt'];
    wd = cd;
    cd(pth);
    fout = fresults;
    foutid = fopen(fout, 'w');
                ...
                for oo = 1 : length(diff_h),
                    ...
                    for nt = 1 : length(noisetypes_list),
                        for ss = 1 : length(snr_list),
                            ...
                            snr0 = zeros(1, length(NormalizedCutOff));
                            snr1 = zeros(1, length(NormalizedCutOff));
                            %                         snr2 = zeros(1, length(NormalizedCutOff));
                            for pm = 1 : length(NormalizedCutOff),
                                ...
                                for trial = 1 : trial_num, % trial is the most internal loop
                                    ...
                                    x = data + noise;
                                    %NormalizedCutOff = NoisePowerOverEstimationFactor*NoisePower;
                                    %                             [~, x_filtered, ] = ECGSmoothnessPriorsDenoiserBW(x, NormalizedCutOff(pm), mode1, diff_h(oo), round(wlen*fs), 1e-8, 250, ForgettingFactor);
                                    ...
                                    %                                 [~, x_filtered] = ECGSmoothnessPriorsDenoiserBW(x, NormalizedCutOff(pm), mode, diff_h(oo), round(wlen*fs), 1e-8, 250, ForgettingFactor);
                                    
                                    dsg = fdesign.lowpass('N,F3db', diff_h(oo), NormalizedCutOff(pm));
                                    % hd = design(dsg,'butter'
                                    % dsg = fdesign.lowpass('N,F3db', 2, .1);
                                    hd = design(dsg,'butter');
                                    [b,a]=sos2tf(hd.sosMatrix, hd.ScaleValues);
                                    x_filtered_LTI = filtfilt(b, a, x);
                                    
                                    % % %                                     x_filtered_LTI = ECGSmoothnessPriorsDenoiserLTI(x, NormalizedCutOff(pm), mode, diff_h(oo));
                            ...
                            [Y I] = max(snr1 - snr0);
                            strng = [num2str(kk) '\t' fname '\t' num2str(nn) '\t' num2str(dd) '\t' num2str(diff_h(oo)) '\t' num2str(nt)  '\t' num2str(ss) '\t' num2str(snr, '%4.4f') '\t' num2str(snr0(I), '%4.4f') '\t' num2str(Y, '%4.4f') '\t' num2str(NormalizedCutOff(I), '%2.6g') '\t' num2str(I, '%d') '\n'];
