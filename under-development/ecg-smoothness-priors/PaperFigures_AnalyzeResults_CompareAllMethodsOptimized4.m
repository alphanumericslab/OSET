...
% results_WAVELET2 = importdata([pth 'AllDataSmootherResultsNormal10_ComparisonWithWavelet3_Backup.txt']);
...
% results_WAVELET2 = importdata([pth 'AllDataSmootherResultsArrhythmia10_ComparisonWithWavelet3_Backup.txt']);
...
results_LPF = importdata([pth 'AllDataSmootherResultsPTB05_ComparisonWithLPF_Backup.txt']);
results_LPF = [zeros(size(results_LPF.data(:,1),1), 2) results_LPF.data(:,1:end)];
results_WAVELET = importdata([pth 'AllDataSmootherResultsPTB06_ComparisonWithWavelet2FewSubjects_Backup.txt']); %% WATCH OUT; THSI IS THE REDUCED SET!
% results_WAVELET = [zeros(size(results_WAVELET.data(:,1),1), 2) results_WAVELET.data(:,1:end)];
results_WAVELET2 = importdata([pth 'AllDataSmootherResultsPTB10_ComparisonWithWavelet3_Backup.txt']);
% results_WAVELET2 = [zeros(size(results_WAVELET2.data(:,1),1), 2) results_WAVELET2.data(:,1:end)];
results_NAIVE = importdata([pth 'AllDataSmootherResultsPTB07_NaiveSweepGammaBasedOptimized_Backup.txt']);
results_NAIVE = [zeros(size(results_NAIVE.data(:,1),1), 2) results_NAIVE.data(:,1:end)];
results_FIXEDVAR = importdata([pth 'AllDataSmootherResultsPTB08_KnownVarianceOptimizedFewSubjects_Backup.txt']); %% WATCH OUT; THSI IS THE REDUCED SET!
results_FIXEDVAR = [zeros(size(results_FIXEDVAR.data(:,1),1), 2) results_FIXEDVAR.data(:,1:end)];
...
snrimp_count_WAVELET = zeros(length(SCAL), length(WNAME), length(snr_list));
snrimp_sqsum_WAVELET = zeros(length(SCAL), length(WNAME), length(snr_list));
snrimp_sum_WAVELET = zeros(length(SCAL), length(WNAME), length(snr_list));
wlevel_sum_WAVELET = zeros(length(SCAL), length(WNAME), length(snr_list));
for k = 1 : length(numeric),
    ...
    if(~isempty(Iscal) && ~isempty(Iwname) && ~isempty(Isnr))
        snrimp_sum_WAVELET(Iscal, Iwname, Isnr) = snrimp_sum_WAVELET(Iscal, Iwname, Isnr) + (snr1 - snr0);
        snrimp_sqsum_WAVELET(Iscal, Iwname, Isnr) = snrimp_sqsum_WAVELET(Iscal, Iwname, Isnr) + (snr1 - snr0)^2;
        wlevel_sum_WAVELET(Iscal, Iwname, Isnr) = wlevel_sum_WAVELET(Iscal, Iwname, Isnr) + level;
        snrimp_count_WAVELET(Iscal, Iwname, Isnr) = snrimp_count_WAVELET(Iscal, Iwname, Isnr) + 1;
    end
end
...
snrimp_mean_averaged_WAVELET = squeeze(mean(snrimp_mean_WAVELET, 3));
snrimp_std_averaged_WAVELET = squeeze(mean(snrimp_std_WAVELET, 3));

[~, II] = sort(snrimp_mean_averaged_WAVELET(:), 'descend');
[scal_opt_WAVELET, wname_opt_WAVELET] = ind2sub(size(snrimp_mean_averaged_WAVELET), II(1));

% Analyze wavelet results (without level maximization)
numeric2 = results_WAVELET2.data;
nominal2 = results_WAVELET2.textdata;

TPTR2 = {'rigrsure'};%%%{'rigrsure', 'heursure', 'sqtwolog', 'minimaxi'};
SORH2 = {'s'};%%%{'s', 'h'};
SCAL2 = {'sln'};%%%{'one', 'sln', 'mln'};
WLEVELS2 = 1 : 10;
WNAME2 = {'haar', 'db2', 'db3' ,'db4', 'db5', 'db6', 'db7', 'db8', 'db9', 'db10', 'db12', 'db16', 'coif1', 'coif2', 'coif3', 'coif4', 'coif5', 'sym2', 'sym3', 'sym4', 'sym5', 'sym6', 'sym7', 'sym8', 'bior1.5', 'bior2.6', 'bior2.8', 'bior5.5', 'bior6.8'};

snrimp_count_WAVELET2 = zeros(length(WNAME2), length(WLEVELS2), length(snr_list));
snrimp_sqsum_WAVELET2 = zeros(length(WNAME2), length(WLEVELS2), length(snr_list));
snrimp_sum_WAVELET2 = zeros(length(WNAME2), length(WLEVELS2), length(snr_list));
wlevel_sum_WAVELET2 = zeros(length(WNAME2), length(WLEVELS2), length(snr_list));
for k = 1 : length(numeric2),
    wname = nominal2(k, 4);
    Iwname = [];
    for j = 1 : length(WNAME2),
        if(isequal(wname, WNAME2(j)))
            Iwname = j;
            break;
        end
    end
    ...
    if(~isempty(Iwname) && ~isempty(Ilevel) && ~isempty(Isnr))
        snrimp_sum_WAVELET2(Iwname, Ilevel, Isnr) = snrimp_sum_WAVELET2(Iwname, Ilevel, Isnr) + snrimp;
        snrimp_sqsum_WAVELET2(Iwname, Ilevel, Isnr) = snrimp_sqsum_WAVELET2(Iwname, Ilevel, Isnr) + snrimp^2;
        wlevel_sum_WAVELET2(Iwname, Ilevel, Isnr) = wlevel_sum_WAVELET2(Iwname, Ilevel, Isnr) + level;
        snrimp_count_WAVELET2(Iwname, Ilevel, Isnr) = snrimp_count_WAVELET2(Iwname, Ilevel, Isnr) + 1;
    end
end

wlevel_mean_WAVELET2 = wlevel_sum_WAVELET2 ./ snrimp_count_WAVELET2;
snrimp_mean_WAVELET2 = snrimp_sum_WAVELET2 ./ snrimp_count_WAVELET2;
snrimp_std_WAVELET2 = sqrt(snrimp_sqsum_WAVELET2./snrimp_count_WAVELET2 - snrimp_mean_WAVELET2.^2);

snrimp_mean_averaged_WAVELET2 = squeeze(mean(snrimp_mean_WAVELET2, 3));
snrimp_std_averaged_WAVELET2 = squeeze(mean(snrimp_std_WAVELET2, 3));

[~, II] = sort(snrimp_mean_averaged_WAVELET2(:), 'descend');
[wname_opt_WAVELET2, wname_opt_WLEVELS2] = ind2sub(size(snrimp_mean_averaged_WAVELET2), II(1));
...
% end
figure(1);
% plot(snr_list, squeeze(snrimp_mean_WAVELET(scal_opt_WAVELET, wname_opt_WAVELET, :)), 'y', 'marker', 'square', 'LineWidth', 3, 'MarkerEdgeColor','k', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
plot(snr_list, squeeze(snrimp_mean_WAVELET2(wname_opt_WAVELET2, wname_opt_WLEVELS2, :)), '--', 'color', 0.4*ones(1,3), 'marker', 'o', 'LineWidth', 3, 'MarkerEdgeColor','m', 'MarkerFaceColor', 'm', 'MarkerSize', 8);
grid
...
% end
figure(2);
% plot(snr_list, squeeze(snrimp_std_WAVELET(scal_opt_WAVELET, wname_opt_WAVELET, :)), 'y', 'marker', 'square', 'LineWidth', 3, 'MarkerEdgeColor','k', 'MarkerFaceColor', 'k', 'MarkerSize', 4);
plot(snr_list, squeeze(snrimp_std_WAVELET2(wname_opt_WAVELET2, wname_opt_WLEVELS2, :)), '--', 'color', 0.4*ones(1,3), 'marker', 'o', 'LineWidth', 3, 'MarkerEdgeColor','m', 'MarkerFaceColor', 'm', 'MarkerSize', 8);
grid
...
figure(1);
legend('Wavelet', 'LPF', 'Fixed \lambda_k', 'Under-estimated \delta_k', 'True \delta_k', 'Over-estimated \delta_k');
axis 'tight';
a = axis;
a(1) = -9;%snr_list(end);
a(2) = 30;%snr_list(1);
a(3) = 0;%-2;%-4;%snr_list(1);
a(4) = 15;%12;%10;%snr_list(1);
axis(a);

figure(2);
legend('Wavelet', 'LPF', 'Fixed \lambda_k', 'Under-estimated \delta_k', 'Known \delta_k', 'Over-estimated \delta_k');
axis 'tight';
a = axis;
a(1) = -9;%snr_list(end);
a(2) = 30;%snr_list(1);
