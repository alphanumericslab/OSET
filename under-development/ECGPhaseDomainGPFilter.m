function [data_posterior_est, data_prior_est, n_var] = ECGPhaseDomainGPFilter(data, peaks, params)
...
for ch = 1 : size(data, 1)
    ...
    if left_padding > 0
        x_stacked = x_stacked(2 : length(II_peaks) - 1, :);
    else
        x_stacked = x_stacked(1 : length(II_peaks) - 1, :);
    end
    X = x_stacked';
    mu_X = mean(X, 2);
    N = size(X, 2);
    ...
    [U,S,V] = svd(Y, 'matrix');
    sigma = diag(S);
    sigma = sigma(1 : end - 1); % remove the last 0 signular value (due to average removal)



    rho_inv = N^2 * n_var ./ sigma.^2;  % <<<=== TODO: Should we have N or N^2 in this equation?



    rho_inv(rho_inv > 1) = 1;
    Lambda = diag([1 - rho_inv; ones(params.bins - length(rho_inv), 1)]);
    Y_filtered = U * (Lambda * S) * V' + mu_X * ones(1, N);

    K_x_ph = cov(x_stacked);
    mu_s = mean(x_stacked, 1);
    K_s_ph = K_x_ph - n_var * eye(size(K_x_ph, 1));
    [U_s,S_s,V_s] = svd(K_s_ph, 'matrix');
    S_s(S_s < 0) = 0;
    K_s_ph = U_s * S_s * V_s;
    MU = ones(size(x_stacked, 1), 1) * mu_s;
    Y_filtered_0 = K_s_ph * pinv(K_x_ph) * (x_stacked - MU)' + MU';
...
grid
hold on
plot(Y_filtered(:, beat))
plot(Y_filtered_0(:, beat))
plot(mu_X)
legend({'X', 'Y_filtered', 'Y_filtered0', 'mu_x'}, 'interpreter', 'none');
