...
def peak_det_simple_unit_test():
    ...
    py = runPython(data)
    ml = runMatLab(data)
    w = isclose(py[0], ml[0], rel_tol=1e-8)
    x = isclose(py[1], ml[1], rel_tol=1e-8)
    y = isclose(py[2], ml[2], rel_tol=1e-8)
    data = [[random.random() for _ in range(1000)] for _ in range(1000)]
    data = np.array(data) * 1000
    py = runPython(data)
    ml = runMatLab(data)
    a = testing.compare_arrays(py[0], ml[0], round_val=8)
    b = testing.compare_arrays(py[1], ml[1], round_val=8)
    c = testing.compare_arrays(py[2], ml[2], round_val=8)
