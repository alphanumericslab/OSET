...
import numpy as np
import scipy.io
from oset.ecg.peak_detection.peak_det_modified_pan_tompkins import (
    peak_det_modified_pan_tompkins,
)
...
def peak_det_modified_pan_tompkins_unit_test():
    ml = run_matLab()
    py = run_python()
...
def run_matLab():
    ...
    return eng.peak_det_modified_pan_tompkins(x, np.double(fs), nargout=3)


def run_python():
    return peak_det_modified_pan_tompkins(mat, fs)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""This is a unit test for peak_det_modified_pan_tompkins"""
    )
    args = parser.parse_args()
    print(peak_det_modified_pan_tompkins_unit_test())
