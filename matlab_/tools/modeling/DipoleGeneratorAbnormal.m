...
for i = 1:N-1
    ...
    X(i+1) = X(i) - dt*sum(w*alphai(state).x ./ (bi(state).x .^ 2) .* dtetaix .* exp(-dtetaix .^2 ./ (2* bi(state).x .^ 2)));   % x state variable
    Y(i+1) = Y(i) - dt*sum(w*alphai(state).y ./ (bi(state).y .^ 2) .* dtetaiy .* exp(-dtetaiy .^2 ./ (2* bi(state).y .^ 2)));   % y state variable
    Z(i+1) = Z(i) - dt*sum(w*alphai(state).z ./ (bi(state).z .^ 2) .* dtetaiz .* exp(-dtetaiz .^2 ./ (2* bi(state).z .^ 2)));   % z state variable
