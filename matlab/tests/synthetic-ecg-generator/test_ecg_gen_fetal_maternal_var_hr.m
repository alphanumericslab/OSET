...
[DIP_m, teta_m] = vcg_gen_var_hr(BPM_m,fs,alphai_m,bi_m,tetai_m,teta0_m, 'MATRIX');
...
[DIP_f, teta_f] = vcg_gen_var_hr(BPM_f,fs,alphai_f,bi_f,tetai_f,teta0_f, 'MATRIX');
...
ns = zeros(dim,N);
for i = 1:dim
    ns(i,:) = biosignal_noise_gen(1,1,0,N,fs,beta,mod(i,2));                     % colored noise with 2 independent dimensions
%     ns(i,:) = biosignal_noise_gen(0,1,0,N,mod(i,3));                           % white noise with 3 independent dimensions
%      ns(i,:) =  biosignal_noise_gen(5,1,0,N,fs,[w_bw,w_em,w_ma],mod(i,8)*1000); % real noise with 8 independent dimensions
end
...
% [A,B,ang] = random_mat_max_col_space_ang(5,dim,3,0);    % maximum subspace angles smaller or equal to 5 degrees 
[A,B,ang] = random_mat_max_col_space_ang(40,dim,3,1); % minimum subspace angles greater or equal to 40 degrees
...
peaks_m = peak_det_local_search(xm(1,1:N),mean(BPM_m)/60/fs);
peaks_f = peak_det_local_search(xf(1,1:N),mean(BPM_m)/60/fs);
Im = find(peaks_m);
If = find(peaks_f);
...
plot_multichannel_data(x,4,'b',fs);
