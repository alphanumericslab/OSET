...
%        - win_qrs: Optional. It is window duration in second unit for extracting diff and std features from ecg signal for QRS onset and offset (default is 0.02 or 20ms)
...
if flag_post_processing>0
    g_windows = min(1,max(0.5,median(avg_intervals_ecg)/fs));
else
    g_windows = 1;
end
...
if nargin > 5 && ~isempty(varargin{3})
    ...
    win_sample_qrs = round(2*sample_10ms*g_windows);
end

if nargin > 6 && ~isempty(varargin{4})
    win_sample_T = round(fs*varargin{4});
else
    win_sample_T = round(3*sample_10ms*g_windows);
end
...
for p = 2:length(ecg_rpeaks_index)-1
    ...
    baseline_intervals{p-1} = (ecg_rpeaks_index(p)-2*sample_70ms:ecg_rpeaks_index(p)-sample_70ms)';
end
...
for p = 2:length(ecg_rpeaks_index)-1
    ...
    max_sample_qrson = min(max(sample_70ms,round(avg_intervals_ecg(p)/6)),max_sample_qrson);
    ...
    if ~isempty(ind_extrm) && ind_extrm>2*sample_10ms && ind_extrm<length(this_qrson_index) - 2*sample_10ms
        ...
        if ind_extrm-sample_win_gain > 2*sample_10ms * min(2,max(0.5,avg_intervals_ecg(p)/fs))
    ...
    if length(this_qrson_index)>sample_70ms
        ...
        if isempty(ind_drop_pq) || ind_drop_pq> ind_extrm-3*sample_10ms || temp(ind_drop_pq)> 5*min(temp)
            ind_drop_pq = 1;
        else
...
for p = 2:length(ecg_rpeaks_index)-1
    ...
    max_sample_qrsoff = min(max(sample_70ms,round(avg_intervals_ecg(p)/6)),max_sample_qrsoff);
    ...
    if ~isempty(ind_extrm)  && ind_extrm>2*sample_10ms && ind_extrm<length(this_qrsoff_index) - 2*sample_10ms
     ...
     if change_sign>0 && first_try>0 && falg_st==1
        this_qrson_index = this_qrson_index(ind_extrm:end);
...
for p = 2:length(ecg_rpeaks_index)-1
    ...
    if abs(ecg_Twave(this_qrs_index(1))) > std_data/2 && abs(ecg_Twave(this_qrs_index(1)) - ecg_Twave(this_qrs_index(end))) > std_data
        ...
        if idx_max<4*sample_10ms && (positions.QRSoff(p) - positions.QRSon(p)) < 3*sample_100ms && (val_max>std_diff || (val_max>std_diff2 && abs(ecg_Twave(this_qrs_index(1))) > std_data && abs(ecg_Twave(this_qrs_index(1)) - ecg_Twave(this_qrs_index(end))) > std_data))
