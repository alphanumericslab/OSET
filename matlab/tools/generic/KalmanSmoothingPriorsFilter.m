function [y1,y2,Pbar,Phat,PSmoothed,Kgain,alpha, y3, PPhat, KKgain, margin] = KalmanSmoothingPriorsFilter(x,R, varargin)
% % % % % % %
% % % % % % % [y1,y2,Pbar,Phat,PSmoothed,Kgain] = KalmanSmoothingPriorsFilter(x,b,a,q,r,gamma,wlen,mode),
...
order = 2;
fs = 1000;
Q = 2e-8*sqrt(fs^order);%1e-2;
% R = 1000;
RVarWinlen = 100;%round(length(x)/10);
QVarWinlen = 100;%round(length(x)/10);
gamma = 0.9;
mode = 'obsvr';
Qadapt = 1;
