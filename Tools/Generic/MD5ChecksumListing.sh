...
root_folder="/path/to/root/folder/for/listing"
output_file="/path/to/output/log/log_file.csv"
# Example:
...
if [[ "$os" == "Linux" ]]; then # FOR LINUX:
    ...
    find "$root_folder" -type f -name "*$file_extension" -exec md5sum {} \; > "$output_file"
