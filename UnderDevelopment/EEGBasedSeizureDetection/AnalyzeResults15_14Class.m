function AnalyzeResults15_14Class(Rdata, Rtextdata, outputfileame1, outputfileame2, ntrees)
% close all;
% % parameters
...
filenames = filenames_all(:); % file names per subject
r = r_raw(:, :); % all features per subject
%     trials = trials_all(subjects); % trials per subject
types = types_all(:); % types (interictal/preictal/test) per subject
...
b = TreeBagger(ntrees, r(train_indexes, :), types(train_indexes) + subjects_all(train_indexes)*10,'OOBPred','on','Method','classification');%,'prior', [0.5 0.5]);%, 'Cost',[0 1.0 ; 1.0 0]);
[Y, scores, ~] = predict(b, r);
...
cc = str2num(cell2mat(Y));
fcolumn = (subjects_all-1)*2 + mod(cc,10);

% log file
outfid1 = fopen(outputfileame1, 'w');
fprintf(outfid1,'clip,preictal\n');
for i = 1:length(test_indexes),
        fprintf(outfid1,'%s,%10.8f\n', filenames{test_indexes(i)}, scores(test_indexes(i), fcolumn(i)));
end
% % % end
fclose(outfid1);
