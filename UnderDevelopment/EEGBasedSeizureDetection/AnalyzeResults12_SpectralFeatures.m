...
for s = 1:max_subject,
    ...
    b = TreeBagger(100, r(train_indexes, :), types(train_indexes),'OOBPred','on','Method','classification','prior', [0.5 0.5]);%, 'Cost',[0 1.0 ; 1.0 0]);
    [estimated_class, scores, stdevs] = predict(b, r);
    ...
    for i = 1:length(test_indexes),
        ...
        fprintf(outfid2,'%s,%10.8f\n', filenames{test_indexes(i)}, max(min(scores(test_indexes(i), preictal_label) - median(scores(test_indexes, preictal_label)) + 0.5, 1.0), 0.0));
        % %         end
    end
end
fclose(outfid1);
