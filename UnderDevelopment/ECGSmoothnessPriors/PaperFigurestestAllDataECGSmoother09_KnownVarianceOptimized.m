% This is the simulation as
% PaperFigurestestAllDataECGSmoother08_KnownVarianceOptimized.m, just with
% an over estimated noise factor
clear;
close all;
clc;
...
for ttp = 1 : length(type),
    switch(type{ttp})
        case 'Normal'
            ...
            diff_h = 2;%[2 4 6];%load DifferentiatorImpulseResponse01 diff_h
            seg_len = 10;
        case 'Arrhythmia'
    ...
    snr_list = 30 : -3: -15;
    wlen = 100e-3;%[100e-3 200e-3];%%%100e-3; % window length (s)
    trial_num = 1;
    NoisePowerOverEstimationFactor = [0.8 1 1.25 1.5];%%%sqrt(fs*wlen)];
    ForgettingFactor = 1;%%%0.9 : 0.1 : 1; % forgetting rate of the smoothness factor calculated in each block
    mode = 3;
    MAX_ITERATION = 100;
    ACCURACY = 1e-8;
    %     LCurveSweepLength = 1;%100;
    fresults = ['AllDataSmootherResults' type{ttp} '09_KnownVarianceOptimized.txt'];
    wd = cd;
    cd(pth);
    fout = fresults;
    foutid = fopen(fout, 'w');
