            ...
            for nt = 1 : length(noisetypes_list),
                for ss = 1 : length(snr_list),
                    ...
                    for trial = 1 : trial_num, % trial is the most internal loop
                        ...
                        [~, x_filtered, optim_gammas1, optim_gammas2] = ECGSmoothnessPriorsDenoiserBW(x, smoothnessparam, mode, diff_h, round(wlen*fs), [], [], ForgettingFactor);
                        %                         [~, x_filtered, optim_gammas1, optim_gammas2] = ECGSmoothnessPriorsDenoiserBW(x, smoothnessparam, mode, diff_h, peaks);
                        x_filtered_LTI1 = ECGSmoothnessPriorsDenoiserLTI(x, median(optim_gammas1), [], diff_h);
                    ...
                    strng = [num2str(snr, '%4.4f') '\t' num2str(snr0, '%4.4f') '\t' num2str(snr1 - snr0, '%4.4f') '\t' num2str(snr2 - snr0, '%4.4f') '\t' num2str(snr3 - snr0, '%4.4f') '\t' num2str(mean(optim_gammas1), '%2.6g') '\t' num2str(mean(optim_gammas2), '%2.6g') '\n'];
                    foutid = fopen(fout, 'a');
                    fprintf(foutid, strng);
                    fclose(foutid);
                    ...
                    title(['SNR0 = ' num2str(snr) '(dB), SNR1imp = ' num2str(snr1 - snr) '(dB), SNR2imp = ' num2str(snr2 - snr) '(dB), SNR3imp = ' num2str(snr3 - snr) '(dB), Params = [' num2str(mean(optim_gammas1)) ', ' num2str(mean(optim_gammas2)) ']']);
                    grid
                end
            end
