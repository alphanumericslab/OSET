...
for ttp = 1 : length(type),
    ...
    trial_num = 1;
    fresults = ['AllDataSmootherResults' type{ttp} '06_ComparisonWithWavelet2.txt'];
    wd = cd;
    cd(pth);
    fout = fresults;
    foutid = fopen(fout, 'w');
                            ...
                            for sorh = 1 : length(SORH),
                                for scal = 1 : length(SCAL),
                                    for wname = 1 : length(WNAME),
                                        ...
                                        [YMAX, IMAX] = max(snr1(tptr, sorh, scal, wname, :) - snr0(tptr, sorh, scal, wname, :));
                                        strng = [num2str(kk) '\t' fname '\t' num2str(nn) '\t' num2str(dd) '\t' TPTR{tptr} '\t' SORH{sorh} '\t' SCAL{scal} '\t' WNAME{wname} '\t' num2str(IMAX) '\t' num2str(nt)  '\t' num2str(snr, '%4.4f') '\t' num2str(snr0(tptr, sorh, scal, wname, IMAX), '%4.4f') '\t' num2str(snr1(tptr, sorh, scal, wname, IMAX), '%4.4f') '\n'];
                                        foutid = fopen(fout, 'a');
                                        fprintf(foutid, strng);
                                        fclose(foutid);
                                    end
