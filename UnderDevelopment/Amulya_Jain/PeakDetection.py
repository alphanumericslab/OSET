...
def peak_detection(x, ff, *args):
    ...
    Inputs:
    x: numpy array, vector of input data
    ff: float, approximate ECG beat-rate in Hertz, normalized by the sampling frequency
    *args: optional arguments
        flag: bool, search for positive (True) or negative (False) peaks. By default,
              the maximum absolute value of the signal determines the peak sign.
        num_rounds: int, the number of iterations to find the R-peaks, up to 3
                    (every time updating the expected R-peak rates). Default = 1 (no iterations)

    Outputs:
    peaks: numpy array, vector of R-peak impulse train
    peak_indexes: numpy array, vector of R-peak indexes
