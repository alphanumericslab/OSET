...
function [pi_0_lsim , coupling_tetha_IM , transition_matrices_IM ,  gmm_para_lsim ,  AIC, log_likelyhood, BIC, pi_steady] = generic_em_lsim( channels_observations , channel_num_states , num_gmm_component , max_itration , extra, init_model)
...
for itr = 1:max_itration
        ...
        for i = 1:channel_num_states(zee)
            ...
            for k = 1:num_gmm_component(zee)
                ...
                if sigma_diag
                    ...
                    weight_update = repmat(temp_gamma_obs(k,:),dim_observation(zee,1),1);
                    sigma_all_new( dimension_numbers_index(zee)+1:dimension_numbers_index(zee+1), 1:dim_observation(zee), i , k) = diag(0.001*var(all_observation(dim_zee,:),[],2,'omitnan')) + ...
                        ((weight_update.*Recentering)*Recentering')/sum(weight_update(1,:));
                end
    ...
    if sigma_diag
        ...
        BIC(itr)= -2*sum(P_O_model) + log(length(temp_targets))* ( C*(C-1) +  sum(channel_num_states(:))^2 - C*sum(channel_num_states(:)) + sum(channel_num_states(:).*num_gmm_component(:)*2.*dim_observation(:)) +  sum(channel_num_states(:).*(num_gmm_component(:)-1))  );
    else
        ...
        BIC(itr)= -2*sum(P_O_model) + log(length(temp_targets))* ( C*(C-1) +  sum(channel_num_states(:))^2 - C*sum(channel_num_states(:)) + sum(channel_num_states(:).*num_gmm_component(:).*(dim_observation(:)+dim_observation(:).^2)) +  sum(channel_num_states(:).*(num_gmm_component(:)-1))  );
